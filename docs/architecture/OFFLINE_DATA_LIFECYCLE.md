...
20 GiB aggregate package size. File paths are relative, traversal-free, lowercase to avoid
case-insensitive filesystem aliases, restricted to a bounded portable character set, and paired
with a canonical bounded media type. Manifest source, version, and jurisdiction labels are
bounded single-line display text. Excess or unsafe metadata fails closed. The manager states
