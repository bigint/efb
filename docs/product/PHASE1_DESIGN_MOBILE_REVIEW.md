...
| DM-07 | P1       | Source-level remediation implemented; native evidence remains open. Night accent controls now use dark text, dedicated day/night high-contrast palettes are selected from iOS Increase Contrast and Android High contrast text, and token tests enforce the documented semantic contrast pairs. No supported physical device has yet confirmed setting-change events, native rendering, screenshots, or sunlight/dark-adaptation behavior. | `packages/design-system/src/tokens.ts`; `packages/design-system/src/tokens.test.ts`; `apps/mobile/src/theme.ts`; themed map layers and controls in `apps/mobile/src/components/MapWorkspace.tsx`                                                                                              | Run the physical-device accessibility matrix in both appearances, toggle the system setting while the app is active, retain screenshots and accessibility-tree evidence, and evaluate glare and dark adaptation.                                                                                                                                                |
...
The source now defines dedicated high-contrast day and night palettes. A module-level
`AccessibilityInfo` subscription follows iOS Increase Contrast and Android High contrast text
without adding a native listener per component. The map background, grid, route, and route
shadow now use the selected semantic palette instead of fixed night colours. Token tests enforce
7:1 for primary, secondary, and accent-control text and 4.5:1 for danger and attention text in
both high-contrast appearances.

This is source-level evidence only. There is no manual cockpit appearance control, and supported
physical devices have not confirmed live setting changes, MapLibre rendering, screenshots, dark
adaptation, or sunlight use. Those checks remain release blockers.
